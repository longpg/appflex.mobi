...
function countPostIDFilterStar(){
	global $wpdb; 
	$link = $wpdb->get_var("SELECT count(p.ID) countFile  FROM  `".$wpdb->prefix."gdsr_data_article` da INNER JOIN $wpdb->posts p ON da.post_id = p.ID WHERE p.post_status = 'publish' and p.post_type='post'");
	return $link;
}
